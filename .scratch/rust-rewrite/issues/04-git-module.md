...
- 驗證(orchestrator, 2026-08-23):解析/timeout/快取初審通過;退回「負面結果不快取」分歧(非 repo 目錄在 1Hz 下每秒 spawn git),已修於 32739a7:Failed 與空 branch 皆寫 ok=0 條目節流、stale fallback 僅限 ok=1、空 branch 回 stale、TimedOut 不寫,decode 對 ok=0 僅接受空 branch。獨立重跑 27+2 全綠。→ done
- 記錄不改:timeout 後 git 子行程不 kill(read-only 無害);空 session_id 的 cwd fallback key 由 ticket 11 主組裝處理。
